"""
快速诊断测试: 隔离订单平滑与情绪调节对成本和SL的影响
测试不同 α 值和情绪配置在低/高 σ_ε 下的表现
"""
import os
import numpy as np
...
def run_single_sim(params, mode='baseline', alpha=0.6, cautious_k=0.0, optimistic_k=0.0):
    """运行单次仿真, 可配置平滑系数和情绪调节系数"""
    ...
    if mode == 'exp2':
        ...
        env.enable_dynamic_events = False
    ...
    for agent_id in env.agent_iter():
        ...
        # 订单平滑 (可配置 α)
        if mode == 'exp2' and env.enable_coordination and k > 1:
...
# 测试配置
configs = [
    {'name': 'Baseline (无平滑无情绪)',         'mode': 'baseline', 'alpha': 0.6, 'ck': 0.0, 'ok': 0.0},
    {'name': '仅平滑 α=0.6',                    'mode': 'exp2',    'alpha': 0.6, 'ck': 0.0, 'ok': 0.0},
    {'name': '仅平滑 α=0.8',                    'mode': 'exp2',    'alpha': 0.8, 'ck': 0.0, 'ok': 0.0},
    {'name': '仅平滑 α=0.85',                   'mode': 'exp2',    'alpha': 0.85, 'ck': 0.0, 'ok': 0.0},
    {'name': '平滑α=0.8 + 谨慎0.2 + 乐观0.1',  'mode': 'exp2',    'alpha': 0.8, 'ck': 0.2, 'ok': 0.1},
    {'name': '平滑α=0.85 + 谨慎0.15 + 乐观0.1','mode': 'exp2',    'alpha': 0.85, 'ck': 0.15, 'ok': 0.1},
]

print("=" * 100)
print("快速诊断测试: 隔离订单平滑与情绪调节对成本和SL的影响")
print("=" * 100)
...
for tc in test_cases:
    ...
    print(f"{'='*100}")
    print(f"{'配置':<35} {'成本':>8} {'SL':>8} {'BWE':>8} {'SL_零售':>8} {'SL_批发':>8} {'SL_分销':>8} {'SL_制造':>8}")
    print("-" * 100)

    base_result = None
    for cfg in configs:
        result = run_single_sim(
            tc['params'], mode=cfg['mode'],
            alpha=cfg['alpha'], cautious_k=cfg['ck'], optimistic_k=cfg['ok'])

        if base_result is None and cfg['name'].startswith('Baseline'):
            base_result = result

        cost_diff = ""
        sl_diff = ""
        if base_result is not None and not cfg['name'].startswith('Baseline'):
        ...
        print(f"{cfg['name']:<35} {result['total_cost']:>8.1f}{cost_diff:>8} "
